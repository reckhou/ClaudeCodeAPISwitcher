...
export function clearClaudeModelSetting() {
  ...
  try {
    ...
    if (err.code !== 'ENOENT') {
      console.error('Failed to clear model from settings.json:', err.message);
    }
  }
}
...
export function buildAnthropicRestoreBlock(apiKey) {
  ...
  const unsetVars = (prefix) => prefix === '$env:'
    ? `$env:ANTHROPIC_BASE_URL = $null; $env:ANTHROPIC_DEFAULT_SONNET_MODEL = $null; $env:ANTHROPIC_DEFAULT_OPUS_MODEL = $null; $env:ANTHROPIC_DEFAULT_HAIKU_MODEL = $null;`
    : `unset ANTHROPIC_BASE_URL; unset ANTHROPIC_DEFAULT_SONNET_MODEL; unset ANTHROPIC_DEFAULT_OPUS_MODEL; unset ANTHROPIC_DEFAULT_HAIKU_MODEL`;

  if (apiKey === null || apiKey === undefined) {
    // No key stored — just unset all override env vars
    if (platform === 'win32') {
      return unsetVars('$env:');
    }
    return unsetVars('');
  }
