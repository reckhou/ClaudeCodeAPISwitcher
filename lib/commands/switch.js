...
export async function switchTo(target) {
  if (target === 'openrouter') {
    ...
    if (defaultModel) {
      console.log(chalk.yellow('⚠ Note: Using /model inside Claude will override your custom model.'));
      console.log(chalk.yellow('  Run `claude-switcher --restore-model` to reapply it.'));
    }

    // j. Windows execution policy warning
